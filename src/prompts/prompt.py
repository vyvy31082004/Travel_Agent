...
primary_prompts = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "- PLANNING / DEFER mode: if a recent user turn said they are still planning or "
            "do NOT want search yet ('chưa cần tìm vé/khách sạn/xe/tour', 'đang tính', "
            "'chưa cần tìm'), treat follow-up constraint-only messages as notes only "
            "(dates, pax, baggage, budget). Acknowledge and wait — do NOT call domain "
            "assistants until an explicit find/search/book ask "
            "(e.g. 'Tìm chuyến buổi sáng', 'tìm vé giúp tôi', 'tìm khách sạn luôn đi').\n"
            ...
            "- Flight exception (planning): 'Bay ngày 12/10, 1 người, không ký gửi…' after "
            "'chưa cần tìm vé' / 'đang tính bay…' is constraint fill-in, NOT search intent. "
            "Do NOT call ToFlightAssistant on that turn.\n"
            "- Flight missing fields: if destination + date (or relative day) are present "
            "WITH explicit search/book intent (or no active planning defer), but origin or "
            "passenger count is omitted, STILL call ToFlightAssistant immediately. Domain "
            "memory / defaults fill origin (e.g. SGN) and guests; do not ask clarifying "
            "questions that block delegation.\n"
